...
//	to make table able to be updated
	...
	private Item createItemFromResultSet(ResultSet rs) throws SQLException {
		return new Item(rs.getString("Item_id"), rs.getString("Item_seller_id"), rs.getString("Item_name"),
				rs.getString("Item_size"), rs.getString("Item_price"), rs.getString("Item_category"),
...
//					(Item_id, Item_seller_id, Item_name, Item_size, Item_price, Item_category, Item_status, Item_wishlist, Item_offer_status)
	 ...
	 * @return A {@code Item} object representing the newly created item, or
	 *         {@code null} if an error occurs.
	 */
	public Item createNewItem(String Item_seller_id, String Item_name, String Item_category, String Item_size,
			String Item_price) {
		...
		Item item = new Item(UUID.randomUUID().toString(), Item_seller_id, Item_name, Item_size, Item_price,
				Item_category, "waiting", "0", "no_offer");
		try (PreparedStatement pstmt = db.getConnection().prepareStatement(query)) {
	 ...
	 */
	public void updateItem(String Item_id, String Item_name, String Item_category, String Item_size,
			String Item_price) {
...
//			remove item from item list
			for (int i = 0; i < itemList.size(); i++) {
				if (itemList.get(i).getItem_id().equals(item_id)) {
					itemList.remove(i);
					break;
				}
			}
	 ...
	 * @return An {@code ObservableList<Item>} containing all offered items for the
	 *         user.
	 */
	 ...
	 */
	public void updateAcceptOffer(String item_id, String buyer_id, String item_price) {
		String query = "UPDATE item SET Item_status = ?, Item_price = ?, Item_offer_status='no_offer' WHERE Item_id = ?";
		String item_status = "bought," + buyer_id;
		try (PreparedStatement pstmt = db.getConnection().prepareStatement(query)) {
			pstmt.setString(1, item_status);
			pstmt.setString(2, item_price);
